//------------------------------------------------------------------
// Code to copy a new MCM font file to the Max7456 OSD eBay Module
// May 2014
//------------------------------------------------------------------
...
void setup()
{
  ...
  spi_transfer(VERTICAL_SYNC_NEXT_VSYNC|OSD_ENABLE);	    // Added 14/5/2014	
  digitalWrite(MAX7456SELECT,HIGH);
  delay(150);	
  ...
  Serial.println("+---------------------------------------------------+");
  Serial.println("  					              ");		
  Serial.println("	     MAX7456 OSD Module Font Changer          ");		
  Serial.println("        For Article in CQ-DATV Edition 12	      ");
  Serial.println("              http://www.cq-datv.mobi/	      ");
  Serial.println("  					              ");	
  Serial.println("          Ready for Font File download              ");
  Serial.println("				                      ");
  Serial.println(" Send the FONT.mcm File Using the Send File Option  ");
  Serial.println("	          				      ");
  Serial.println("					              ");
  Serial.println(" During download do not distrurb the OSD Module     ");
  Serial.println("					              ");
  Serial.println("+---------------------------------------------------+");
  delay(100);  
}
...
void loop()
{
  ...
  pinMode(MAX7456SELECT,HIGH);
   ...
   for (x = 0; x < MAX_font_rom; x++)
   {
    ...
    Serial.println("");
    Serial.println("Done with font file download");
    Serial.println("");
    Serial.println("You can now use the Test code to create the captions you require.");
  }
}
